(function () {
    ...
    async function handleSearchSubmit(e) {
        ...
        // send request
        axios.get(`https://www.metaweather.com/api/location/search/?query=${inputVal}`)
            .then(response => {
                data = response.data;
                if (data[0]) {
                    woeid = data[0].woeid;
                    sendRequest(`https://www.metaweather.com/api/location/${woeid}/`, 'true');
    ...
    function getCityWeather(woeid, longLat) {
        document.querySelector('#no-data').classList.add('d-none');
        if (!longLat && !woeid) {
            sendRequest(`https://geolocation-db.com/json/`, 'nolong-nowoend');
        }

        if (!woeid) {
            sendRequest(`https://www.metaweather.com/api/location/search/?lattlong=${latLong}`, 'nowoend');
        }
    ...
    // -----------------------------------------------------------------------
    function sendRequest(url, caseState) {
        axios.get(url)
            .then(response => {
                data = response.data;
                if (caseState == 'nolong-nowoend') {
                    latLong = `${data.latitude},${data.longitude}`;
                    sendRequest(`https://www.metaweather.com/api/location/search/?lattlong=${latLong}`, 'nowoend');
                } else if (caseState == 'nowoend') {
                    woeid = data[0].woeid;
                    sendRequest(`https://www.metaweather.com/api/location/${woeid}/`, 'true')
                } else if (caseState == 'true') {
                    weatherObj = data;
                    changeHTMLData();
                }
            })
    ...
    // -----------------------------------------------------------------------
    // App close and minimize buttons
    // -----------------------------------------------------------------------

    document.querySelector('#minimizeApp').addEventListener('click', function() {
        ipcRenderer.send('minimize-app');
    });

    document.querySelector('#closeApp').addEventListener('click', function() {
